...
public class FieldOfVision : MonoBehaviour
{
    ...
    void DrawFieldOfView()
    {
        ...
        for (int i = 0; i <= stepCount; i++)
        {
            ...
            if(i > 0)
            {
                bool edgeDistThresholdExeeded = Mathf.Abs(oldViewCast.dist - newViewCast.dist) > edgeDistThreshold;
                if(oldViewCast.hit != newViewCast.hit || (oldViewCast.hit && newViewCast.hit && edgeDistThresholdExeeded))
                {
